...
let { enabled: intervalEnabled, time: intervalTime } = require("config").get(
  "interval"
);
...
console.log("Se imprime el nunero aleatorio: " + enteroAleatorio);
intervalTime = intervalTime + enteroAleatorio;
console.log("Se imprime la variable intervalTime: " + intervalTime);
...
const main = async _ => {
  ...
  const confirmNotificationDialog = async browser => {
    const confirmNotification = await browser.elementExist(
      "css",
      {
  ...
  const checkInsufficientBalanceAlert = async browser => {
    const alertDialog = await browser.elementExist(
      "css",
      {
        css:
          "body > uni-app > uni-toast > div.uni-toast > i.uni-icon-error + p.uni-toast__content"
      },
      defaultTimeout
    );
  ...
  const login = async (
    browser,
    { routine = "login", page = "login", retry = routineRetries } = {}
  ) => {
    try {
      await browser.loadUrl(
        "https://www.cotps.com/#/pages/login/login?originSource=userCenter"
      );
      const regionsDropdown = await browser.findElement("css", {
        css:
          "body > uni-app > uni-page > uni-page-wrapper > uni-page-body > uni-view > uni-view:nth-child(5) > uni-text"
      });
      ...
      const phoneNoInput = await browser.findElement("css", {
        css:
          "body > uni-app > uni-page > uni-page-wrapper > uni-page-body > uni-view > uni-view:nth-child(5) > uni-input > div > input"
      });
      ...
      const passwordInput = await browser.findElement("css", {
        css:
          "body > uni-app > uni-page > uni-page-wrapper > uni-page-body > uni-view > uni-view:nth-child(7) > uni-input > div > input"
      });
      ...
      await browser.waitUntilUrlIs(
        "https://www.cotps.com/#/pages/userCenter/userCenter"
      );
  ...
  const loadTransactionHall = async (
    browser,
    {
      routine = "loadTransactionHall",
      page = "loadTransactionHall",
      retry = routineRetries
    } = {}
  ) => {
    try {
      await browser.waitUntilUrlIs(
      ...
      const transactionHallBtn = await browser.findElement("css", {
        css:
          "body > uni-app > uni-tabbar > div.uni-tabbar > div:nth-child(3) > div"
      });
      await browser.click(transactionHallBtn);

      await browser.waitUntilUrlIs(
        "https://www.cotps.com/#/pages/transaction/transaction"
      );
      await browser.waitUntilPageIsLoaded();
    ...
    try {
      ...
      do {
        const findOrdersBtn = await browser.findElement("css", {
          css:
            "body > uni-app > uni-page > uni-page-wrapper > uni-page-body > uni-view > uni-view.grab-orders-wrap.grab-orders-wrap1 > uni-button"
        });
  ...
  const logout = async (
    browser,
    { routine = "logout", page = "mine", retry = routineRetries } = {}
  ) => {
    try {
      await browser.refresh();
      const mineBtn = await browser.findElement("css", {
        css:
          "body > uni-app > uni-tabbar > div.uni-tabbar > div:nth-child(5) > div"
      });
      ...
      const logoutBtn = await browser.findElement("css", {
        css:
          "body > uni-app > uni-page > uni-page-wrapper > uni-page-body > uni-view > uni-view.box-wrap > uni-button"
      });
  ...
  try {
    ...
    fs.rmSync(path.resolve(__dirname, "./evidence"), {
      recursive: true,
      force: true
    });
    await login(browser);
    ...
    console.log(
      "-------------Iniciara el otro Trabajo en: ",
      moment().add(intervalTime, "m").format("DD/MM/YYYY hh:mm a")
    );
  } catch (err) {
    console.error(err);
    await browser.quit();
...
(async function invoker() {
  if (intervalEnabled) {
    ...
    await main().then(console.log).catch(console.error);
  }
})();
